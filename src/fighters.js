...
`
  ...
  /**
   * 循环动作里真正要"锁住姿势、不许走跑接管"的白名单。
   *
   * ⚠ 这里**故意是空的**：`guard_infinity`（无下限 / 闪避的防御姿势）一开始被我放进来，
   * 结果模糊测试抓到"角色一边以 4 m/s 滑行 2.6 米、一边保持防御姿势不动"，
   * 大腿摆幅只有 0.09 rad —— 正是用户说的"点了闪避之后动作僵硬不变"。
   * 防御姿势本身是循环动作，而它要等 INFINITY_TIME 结束才被 setGuard(false) 收掉，
   * 这段时间玩家是可以自由移动的，所以"能走就必须让走跑接管"。
  ...
  function createFighter(who, opts) {
    ...
    const play2 = (name, o) => {
      ...
      /**
       * ⚠ 早退条件必须带上 curLoop：同一个动作名可能先用 loop:false 播（一次性起手），
       * 再被要求 loop:true（持续状态）。只看 loop3 就早退的话，curLoop 会**永远留在 false**，
       * 结果这个"应该一直循环"的动作播到 dur 就停住 —— 走/跑的动画卡在最后一帧不动，
       * 正是"点了疾跑之后动作僵硬不变"。
       */
      if (name === curName && loop3 && curLoop) {
    ...
    const moveTowards = (x, z, speed, dt) => {
      ...
      if (curName === "idle" || curName === "walk" || curName === "run" || oneShotDone || loopingForeign) {
         ...
         * 实测抓到过 1.4 秒的窗口（用户原话："动作僵硬不变"）。
         * 出招期间不会受影响：那时候 curName 是招式动作，上面的守卫已经拦住了。
         */
        if (wantAnim === "idle" && want > 0.05) wantAnim = "walk";
        /**
         * ⚠ 换步态必须有稳定期（0.1s），不能一帧一换。
         * play2() 每次换名字都会把 curT 和 blendT 归零，如果 wantAnim 在阈值附近来回跳
         * （实测：出招降速后 want 在 1.6~4.0 之间摆动），就会变成"每帧重新起手"——
         * 混合权重永远停在 0，骨骼几乎不动，看起来就是**动作僵硬不变**。
         */
        /**
         * 用"切换冷却"而不是"目标稳定计时"来防抖。
         * 上一版写成 gaitHold += dt 直到 0.1s 才切，结果 wantAnim 在阈值附近
         * 一帧 walk 一帧 idle 地抖时，gaitHold 每帧都被 else 分支清零，
    ...
    const setGuard = (on) => {
      ...
      applyAuraMats();
      if (guardOn) {
        play2("guard_infinity", { loop: true, speed: 0.9 });
      } else if (curName === "guard_infinity" || curName === "block" || curName === "block_hit") {
        // 关掉防御时必须把姿势收回去：guard_infinity 是循环动作，
        // 不主动切走的话角色会一直保持防御姿势，移动时表现为"滑着走、腿不摆"。
        play2("idle", { loop: true });
      }
    };
    const setAura = (on) => {
