...
**在空中**（⚠ 2026-09-12 修订：高度从 14~17m 下调到 4.8~6.6m，原因见下）
- **基准高度 AIR_Y = 4.0 / 4.5 / 5.0（easy/normal/hard）**（2026-09-12 最终值；模型高 4.076m，
  摆动的锚点实测 4.08~5.0，绝大多数帧 4.0~5.0），绕宿傩 ±4.5m 侧向往返，水平速度 8~12 m/s。
  配合 ```camFrame```（frac 0.085 / pitch 0.26）把机位拉远，作者自测 150 帧「脚底 + 法轮顶两端」insideBoth 100%。
- 为什么必须下调：上帝机位是「俯视交战点」的设计，实测（1600x900，典型交战距离）把玩家正上方的点投影到 NDC 得到
  y=5 → ndcY 0.78、y=6 → 0.98、y=7 → 1.20（出画）。也就是说**地面上方 6.2m 以上在正常战斗中完全看不见**，
  原来写的 14~17m 会投影到 ndcY 3.2~4.0，是屏幕高度的 1.6~2 倍以上 —— 玩家根本看不到魔虚罗。
  修正后 ndcY ≈ 0.85，落在画面上沿内，且离地 5.8m（约 3 个角色高）近战仍然够不到（近战扫掠在 y≈1.4，3D 判定差 4m+）。
- **可见性验收**：真实战斗中（含镜头推近/拉远）采样 ≥95% 的帧，魔虚罗胸口投影 |ndcY| ≤ 0.92 且 |ndcX| ≤ 0.95；
  出画时必须有一个自己的「↑ 魔虚罗 · 距离」屏外指示（不要依赖 hud.js 的罗盘，那是给宿傩用的）。
...
- **判定（2026-09-12 最终版 · 帧率无关的时间窗）**：命中时刻 `tF = cb.time - 本帧 dt`（**命中帧起始**）、
  按下时刻 `tP = cb.pressTime.v`（main.js 用 keydown 的 performance.now() 做亚帧折算写入）。
  `ok = |tF - tP| <= W`，W = 0.0167s（桌面）/ 0.033s（触屏），连闪 >=2 与「咒力掌握」期各 +0.0167s（上限 0.033s）。
  **为什么不能用帧判据**：本作 rAF 跑在显示器刷新率上（实测无头环境 139fps）。
  「±1 帧」在 144Hz 下只有 ±7.2ms、60fps 下才有 ±16.7ms —— 同一套手速在 144Hz 上难一倍，那是不公平而不是难度。
  改成时间窗后实测「真实按下时刻被接受的区间」：30fps 18ms / 60fps 16ms / 144fps 16ms（三档差 2ms 以内）。
  顿帧期间 `cb.time` 冻结（update2 早退），时间窗在顿帧里天然等价于「同帧」。
  因果限制（不要求实现）：命中后 8ms 的按下不可能对**同一次**命中生效（命中已在命中帧内结算）。
...
- **镜头**：FOV 随速度变化（**实测基线 44°，不是 60°**，验收按 **Δ ≥ 6°** 判）；相机稍后拉；高速时 `render.impulse({radialBlur})`。
