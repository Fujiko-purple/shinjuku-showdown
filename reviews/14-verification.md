...
最后更新：22:0x —— **最终版，全部结论以合流后的 `dist/新宿决战.html` 为准**（§0 是总结，§3 里带 WIP 标注的是过程记录）。
注意：§3.1~§3.4 里逐节保留了我发现 / 撤回 / 改判的完整过程（含我自己的测量方法修正 §4.1），方便对账；最终判定一律看 §0。

---

## 0. 最终结论（对合流后的 dist/新宿决战.html，2026-09-12 22:0x）

| 机制 | 脚本 | 结果 | 关键数字 |
|---|---|---|---|
| §5 领域对决 | `verify-duel.mjs --n 30` | **16/16 PASS** | 连点 30/30 判负、精确对齐 30/30 胜（契约 ≥28）、挂机 2.23s 判负、输局 hp 1800→1447.9 + burnout 4.935、needle∈[-1,1]（1211 帧 0 越界）、窗口 0.30→0.18（每回合 -0.03）、半周期最快 0.796s（往返 1.591s）、窗口重随机 8/8 都在指针穿过之后 |
| §7 疾跑（桌面） | `verify-sprint.mjs` | **26/26 PASS** | 0.35s v=9.891、稳态 10.493、1s 位移 10.50m、FOV 44→51.99（Δ7.99）、phase=sprint、dust=6、松手惯性 +0ms 10.808→+0.25s 6.362、走路 4.839、乱按/挂机/折返全过 |
| §7 疾跑（手机 844x390 low） | `verify-sprint.mjs --mobile` | **11/11 PASS** | 摇杆(0,-1) v=10.786 phase=sprint run=true；0.90 不疾跑；0.95 疾跑；回中不位移 |
| §6 黑闪 | `verify-bf.mjs` | **15/16 PASS** | 时间窗自洽 ok===dtMs<=winMs（窗口外 7 例全失败、窗口内 2 例全成功：14.9/16.7ms、29.7/33ms）、随机按 V 500 次→0、每帧按 V+连打→0（14 次普通命中照打）、伤害真值比 **2.500**（6.50/2.60）、事件带 blackFlash、stats+1 |
| §4 魔虚罗 | `verify-mahoraga.mjs` | **全部语义断言 PASS** | 50% 边界触发/不重复触发、phaseLock 2.80、锚点 15.16m、躯体高 3.83m、可见 23/24、draw +55（≤70）、悬浮近战 0 伤、未锁定 0 伤、锁定后赫 3/10 茈 2/10 命中、适应 115→52→52→0、未挡下单发 ×0.600、宿傩打玩家 ×1.250、帧时间 p95 7.1→8.6 |
| 回归 | `verify-regression.mjs` | **8/8 PASS** | smoke / acceptance / probe-movedir / dash / bgm-dom / hudcollide / mobilecase / frametime 全部 exit 0 |
| 主观审查 | `verify-art.mjs` | **4 机制 PASS（1 中性）** | 13 张截图；魔虚罗/领域同步轴/黑闪收缩环都清楚可读；疾跑静帧看不出速度感（数值达标） |

**仍然存在的问题（逐条判定）**

1. **契约 §4「基准高度 y ≈ 14~17」与实测不符（不是缺陷，是契约没同步）**：实测锚点 y 在 4.08~15.16 之间摆动、绝大多数帧 4.0~5.0（Lead 说明 AIR_Y 已改成 4.0/4.5/5.0 取景）。**判定：实现 PASS / 契约 §4 需要 Lead 更新文本**，否则按冻结契约字面就是 FAIL。
2. **黑闪那条 15/16 里的 FAIL 是我自己的过时断言**：契约 §6 写「±3 帧失败」，但 Lead 已把判定改成帧率无关的时间窗（16.7ms / 触屏 33ms / 连闪掌握期放宽），139fps 下 4 帧只有 29.7ms，落在放宽后的 33ms 窗口内、成功是**正确**的。我已在脚本里删掉帧口径断言，换成「ok === dtMs <= winMs」，同一批数据里通过。
3. **观察（未判 FAIL，供 Lead 决策）**：魔虚罗「挡下」实测 7/12 次（声明 30%），导致玩家→宿傩的**合计**伤害比只有 0.292（契约 §4 的 damageGate ×0.6 用「未挡下单发中位」验是 0.600 ✓）。如果用户要的「对玩家伤害免疫更多」就是这个强度，没问题；如果 30% 是目标值，需要作者复核触发条件。
4. **观察**：普通轻击伤害偶尔是 2.99 而不是基准 2.60（18 次里 1~3 次），黑闪倍率因此必须用「多次普通命中的中位」做分母（我用过的取第一发样本会误报 2.174）。
---
...
5. **最终 dist（合流后重建版）正式数字**：

       node _tools/verify-duel.mjs --file dist/新宿决战.html --port 9514 --n 30
       连点汇总: 开局 30/30 判负 30 判胜 0        → PASS §5-1（契约要求 30/30 失败）
       对齐汇总: 开局 30/30 判胜 30 胜率=100.0%   → PASS §5-2（契约要求 30 次 >= 28 胜）
       逐回合: 窗口宽 0.30 -> 0.27 -> 0.24 -> 0.21 -> 0.18（每回合 -0.03，契约 0.30 -> 0.12）
       PASS §5 指针随回合加快（T 估计 12.14s -> 9.94s，含定格）
       PASS §5-3 站着不动判负 | lose elapsed=2.228s
       PASS §5-4 玩家输：扣血 1447.93 + burnoutT=4.935
       PASS §5 needle 始终在 [-1,1] | 样本=1211 越界=0
       全部半周期样本 s=[1.577,0.798,1.445,0.796,1.506,0.806] 最快=0.796s
       PASS §5 首回合 needle 往返 ≈ 1.6s | 最快半周期=0.796s（往返 1.591s）
       PASS §5 窗口只在指针经过后重新随机 | 重随机=8 通过=8
       ===== duel 结果 16/16 PASS, 0 FAIL (308.1s) =====

   §5 最终：**16/16 PASS（0 FAIL）**。
...
### 3.6 §7 疾跑 —— 最终 dist（合流重建版）**26/26 PASS**

    node _tools/verify-sprint.mjs --file dist/新宿决战.html --port 9512
    ===== sprint 结果 26/26 PASS, 0 FAIL (29.4s) =====

    PASS §7 0.35s 内升到 10.5 量级 | v(0.35s)=9.891
    PASS §7 稳态速度落在 10.5±1 | v(稳态)=10.493
    PASS §7 起步有加速过程 | v(0.1s)=5.734 vs 稳态 10.493
    PASS §7 1s 位移明显大于旧版 4.8m/s | 1s 位移=10.50m
    PASS §7 FOV 变化 >= 6 度 | idle=44 -> max=51.99 Δ=7.99
    PASS §7 相位读到 sprint | phases=["walk","run","sprint"]
    PASS §7 扬尘 > 0 | dust max=6
    PASS §7 松手不瞬间归零 | v(+0)=10.808；松手 0.25s 回落到 6.362
    PASS 走路速度仍是 4.8 量级 | v(走路)=4.839
    PASS D/E/F/G/H 反例与边界（乱按、每帧按抬、高频 toggle、挂机、0.35s 前后）全部通过

**速度上限口径已按 sprint 的对账证据修正**：原来用「逐帧墙钟速度 ≤ 12」判，在高刷（--disable-frame-rate-limit）下没有判别力
（sprint 给的走路对照组：真速度恒 4.8 m/s，逐帧峰值也读到 10.44）。改成「≥1s 位移均速 ≤ 12」后：
E 段 1s 均速 6.493、G 段 1s 均速 8.806，全部合规（逐帧值仅作诊断打印）。
### 3.7 §7 手机端（844x390 + html.is-touch + 触摸模拟）—— 最终 **11/11 PASS**

    node _tools/verify-sprint.mjs --file dist/新宿决战.html --port 9512 --mobile
    ===== sprint-mobile 结果 11/11 PASS, 0 FAIL (18.3s) =====

    手机 摇杆到底(0,-1): v(0.9~1.4s)=10.786 phase=sprint run标志=true
    PASS §7 手机摇杆推到底触发疾跑 | v=10.786
    PASS §7 手机 phase=sprint | phases=["walk","run","sprint"]
    PASS §7 __TOUCH.run 标志被补上 | run 采样值=[false,true]
    手机 摇杆 0.90: v=4.101 sprint=false → PASS 边界 0.90 不疾跑
    手机 摇杆 0.95: sprint=true → PASS 边界 0.95 触发疾跑
    PASS 手机 摇杆回中不位移 | 位移=0.000m
    quality=low（844x390 走低画质路径，全程无报错）

**这条 FAIL 我原来的判定是对的，根因也已经查清并修掉**：我直接写 @@window.__TOUCH.{on,mx,mz}@@（main.js 的 buildInput 会读这个桥），
但「|摇杆| > 0.92 → 疾跑」的派生当时只在 mobile.js 的 syncBridge 里做一次，直写桥绕过了它 —— 位移是正常走路 4.9 m/s，
只是没人派生 run/Shift。sprint 已把派生改成每帧从桥派生（src/mobile.js，他的写域），修后同一条命令 11/11。
另外「0.90 边界」那条我原来把上一条用例松档后的 0.25s **惯性尾巴**算进了采样窗（契约 §7 要求惯性回落），
已在脚本里先沉降 600ms 再采样。
