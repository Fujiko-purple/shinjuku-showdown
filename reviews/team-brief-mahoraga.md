...
【工作方式】
- 共享任务板上你的任务是 task-2：先 team_task_get 拿 revision → team_task_update claim → 做完 complete。
...
【你的模块：task-2 · 魔虚罗 + 二阶段宿傩 · 写域 src/mahoraga.js + _tools/probe-mahoraga.mjs】
规格在契约 §4，逐条照做。重点提醒：

1. **建模是用户直接点名的交付物**（原话「会出现魔虚罗建模」），必须经得起挑剔：4m 高、
   深色躯体 + 骨白护甲 + 暗红发光眼 + 背后退魔之剑 + 头顶八握剑异戒神将**法轮（8 格）**。
   参考 src/fighters.js 里程序化建模的写法（几何体拼装 + 关节分组 + play(name) 姿态），
   但**不要改 fighters.js**（那是 sprint 队友的写域）—— 在你的文件里自建轻量 rig。
   用截图逐个角度自查（_tools/cdp.mjs 的 screenshot 配合 __SS.cam 摆机位）。
2. 契约 §4 里「验收」的每一条都要有对应的探针断言，并把数字贴进报告。
3. 玩家打不到时要有明确解释（横幅/提示「它悬在空中」），俯冲落地 1.6s 的弱点窗口要一眼可见。
4. 平衡红线：**不要改 TUNE / SKILL_DATA 里既有数值**，你的倍率都写在 mahoraga.js 顶部的常量表里，
   并在文件头注释里列出这张表（用户明确要求不要动整体数值设计）。
5. 至少保证：宿傩半血触发；4 个技能各有前摇与可躲窗口；适应 2 次 -55% / 4 次免疫；
   庇护期间玩家打宿傩 ×0.6、宿傩打玩家 ×1.25；击破后有明确演出与状态变化。
